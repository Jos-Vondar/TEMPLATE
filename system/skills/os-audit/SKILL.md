...
- **Deux couches de règles.** `~/.claude/CLAUDE.md` est la couche *règlement*, payée à chaque session ; `~/.claude/fiches/` porte des fiches *situationnelles* chargées sur déclencheur nommé. **Leur nombre se compte dans le dossier et dans la table des déclencheurs, jamais ici** — un compteur recopié se périme. Auditer les deux : une fiche non routée ne se charge jamais, et une règle extraite dont le déclencheur ne couvre pas le moment du danger est invisible précisément quand elle protège.
...
**Et un détecteur reste entièrement manuel** : vérifier que les fiches ne sont pas préchargées. Aucun script ne peut le faire — il ne voit pas ce que l'outil injecte dans une session. Le geste : ouvrir une session neuve et chercher le contenu d'une fiche dans le contexte reçu. Présent = le corpus entier est repayé à chaque session sans que rien ne le signale. À refaire après toute mise à jour de Claude Code, et à la première ouverture sur une machine neuve.
...
10. **Le chargement des fiches est-il toujours paresseux ?** L'audit ne peut pas le vérifier lui-même : il tourne dans une session déjà ouverte, et le fait ne se lit qu'à l'ouverture. Il le **rappelle**, et le rapport porte la réponse de la dernière ouverture connue. Ce détecteur n'a aucun mécanisme : il n'existe que si quelqu'un s'en souvient.
...
2. **Table des artefacts** → chaque chemin cité existe. Sens inverse, c'est l'invariant du système : tout dossier de premier niveau réel et tout fichier de mémoire doit être atteignable depuis cette table. Vérifier aussi que chaque fiche est routée et que chaque fiche citée existe — **une fiche orpheline ne se charge jamais**.
...
6. **Outillage externe en retard.** Le système dépend d'outils tiers qui bougent sans prévenir, et **rien ne surveille leur version** : ni la plomberie, ni le démarrage. Comparer **l'installé au publié**, et donner l'écart en versions, pas seulement « à jour / en retard ».
   - **Le proxy économe** : version locale par `rtk --version`, dernière publiée sur `github.com/rtk-ai/rtk/releases`. Mise à jour par le même script d'installation. **Signaler dans la même ligne que `rtk init -g` écrase `~/.claude/RTK.md`** et qu'il faut le restaurer depuis le dépôt juste après — la version livrée par l'outil pèse plusieurs fois la nôtre sur une couche chargée à chaque session.
