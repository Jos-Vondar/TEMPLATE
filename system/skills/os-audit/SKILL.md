...
5. **Distillation** — relire une semaine de journal, de reprises et de retours, et proposer les règles candidates : proposition, validation, écriture. **Jamais d'auto-règle.** Et appliquer **le troc** du règlement (`CLAUDE.md` §1, s'il a été retenu à l'entretien) — c'est ici que se nomme ce qui sort. Inscrire ensuite la semaine courante dans le marqueur de distillation.
